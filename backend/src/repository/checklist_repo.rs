...
pub async fn create_task_list(
    ...
    Ok(sqlx::query_as(
        "INSERT INTO task_lists (card_id, name, position) VALUES ($1, $2, $3) RETURNING *",
    )
    .bind(card_id)
    .bind(name)
    .bind(position)
...
pub async fn task_list_by_id(pool: &sqlx::PgPool, id: &Uuid) -> Result<Option<TaskList>, AppError> {
...
pub async fn update_task_list_name(pool: &sqlx::PgPool, id: &Uuid, name: &str) -> Result<(), AppError> {
    sqlx::query("UPDATE task_lists SET name = $1, updated_at = NOW() WHERE id = $2")
        .bind(name).bind(id).execute(pool).await?;
    Ok(())
}

pub async fn delete_task_list(pool: &sqlx::PgPool, id: &Uuid) -> Result<(), AppError> {
...
pub async fn create_task(
    ...
    Ok(sqlx::query_as(
        "INSERT INTO tasks (task_list_id, name, position) VALUES ($1, $2, $3) RETURNING *",
    )
    .bind(task_list_id)
    .bind(name)
    .bind(position)
...
pub async fn task_by_id(pool: &sqlx::PgPool, id: &Uuid) -> Result<Option<Task>, AppError> {
...
pub async fn delete_task(pool: &sqlx::PgPool, id: &Uuid) -> Result<(), AppError> {
    sqlx::query("DELETE FROM tasks WHERE id = $1")
        .bind(id).execute(pool).await?;
    Ok(())
}

pub async fn list_by_card(pool: &sqlx::PgPool, card_id: &Uuid) -> Result<Vec<TaskListWithTasks>, AppError> {
