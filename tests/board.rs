...
async fn test_search_returns_json() {
    ...
    assert!(content_type.starts_with("application/json"), "search should return JSON, got: {content_type}");
    let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let data: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    let board_names: Vec<&str> = data["boards"].as_array().unwrap().iter()
        .filter_map(|b| b["name"].as_str()).collect();
    assert!(board_names.contains(&"Alpha Board"), "empty query should return all boards");
    assert!(board_names.contains(&"Beta Project"), "empty query should return all boards");
}
