<script lang="ts">
	...
	function startEditName() {
		if (card) { editName = card.name; editingName = true; }
	}

	function startEditDesc() {
		if (card) { editDesc = card.description ?? ''; editingDesc = true; }
	}
		...
		<div class="panel-body">
			...
			{:else if card}
				...
				<section>
					<h3>Description</h3>
					{#if editingDesc}
						<textarea class="desc-textarea" bind:value={editDesc} onblur={saveDescription} disabled={saving} rows={4}></textarea>
					{:else}
						<button class="desc-btn" onclick={startEditDesc}>{card.description || 'Add a description...'}</button>
					{/if}
				</section>
				...
				{#if card.checklists.length > 0}
					<section>
						<h3>Checklists</h3>
						{#each card.checklists as tl (tl.id)}
							<div class="checklist">
								<h4>{tl.name}</h4>
								{#each tl.tasks as task (task.id)}
									<label class="task">
										<input type="checkbox" checked={task.is_completed} onchange={() => toggleTask(task, tl)} />
				...
				{#if actions.length > 0}
					<section>
						...
						<ul class="activity-list">
							{#each actions as act}
								<li>{act.type} — {formatDate(act.created_at)}</li>
							{/each}
						</ul>
					</section>
				{/if}
